...
export default async function UserPage({ params }: UserPageProps) {
  ...
  const { data: packages } = await searchPackages({ q: username, limit: 50 }).catch(() => ({ data: [] }));
      ...
      <div className="flex flex-col gap-6 border-b border-border py-10 sm:flex-row sm:items-start">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <h1 className="font-mono text-[26px] font-semibold tracking-[-0.02em]">{username}</h1>
            {user.verified && (
              <span
          ...
          {user.bio && (
            <p className="mt-2 max-w-[560px] text-[14.5px] text-fg-muted">{user.bio}</p>
          )}
          ...
          <div className="mt-5 flex flex-wrap gap-6">
            {[
              { Icon: Package,  label: 'Packages',  value: String(packages.length) },
              { Icon: Download, label: 'Downloads',  value: fmtNum(totalDownloads) },
